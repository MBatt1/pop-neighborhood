...
function fillGrid() {
    ...
    var frontier = [{x:startX, y:startY}]
    ...
    while (frontier.length > 0) {
        ...
        if (front.y % 2 == 0) {
            if (front.y > 0) {
                frontier.push({x:front.x-1, y:front.y-1});
                grid[front.y-1][front.x-1] = randomArrayChoice(frontTile.validConnections.u1);
            }
        } else {
            if (front.y > 0) {
                frontier.push({x:front.x, y:front.y-1});
                grid[front.y-1][front.x] = randomArrayChoice(frontTile.validConnections.u1);
            }
        }
    }
    ...
    for (var yPlace = 0; yPlace < screenTileHeight; yPlace++) {
        for (var xPlace = 0; xPlace < screenTileWidth; xPlace++) {
            //if (grid[yPlace][xPlace] == 0) continue;
            var xOffset = xPlace*200;
            if (yPlace % 2 == 0) {
                xOffset -= 100;
            }
            ctx.drawImage(randomArrayChoice(tiles).image, xOffset, yPlace*50-250);
